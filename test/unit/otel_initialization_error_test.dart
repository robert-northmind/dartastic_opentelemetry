...
void main() {
  ...
  test('tracerProvider() after API auto-install names the offending factory',
    ...
    expect(
      ...
      throwsA(isA<StateError>().having(
        ...
        'message',
        allOf(
          contains('auto-installed its no-op factory'),
          contains('Ensure OTel.initialize() runs before any SDK accessors'),
          contains('In tests, call OTel.reset()'),
        ),
      )),
    );
  });

  test('initialize() after API tracerProvider() names the offending factory',
      () async {
    api.OTelAPI.tracerProvider();
    await expectLater(
      OTel.initialize(),
  ...
  test('initialize() after API meterProvider() names the offending factory',
      () async {
  ...
  test('initialize() after API loggerProvider() names the offending factory',
      () async {
  ...
  test('loggerProvider() before initialize() throws the same clear error', () {
    expect(
      OTel.loggerProvider,
      throwsA(isA<StateError>().having(
        (e) => e.message,
